...
type ContactEnv = { RESEND_API_KEY?: string; CONTACT_EMAIL?: string; TURNSTILE_SECRET_KEY?: string };
...
export async function POST(request: Request) {
  ...
  if (origin && host && new URL(origin).host !== host) return json({ error: "Invalid origin" }, 403);
  ...
  if (!name || !message || !EMAIL_PATTERN.test(email)) return json({ error: "Invalid form data" }, 400);

  const { env } = await getCloudflareContext({ async: true });
  const { RESEND_API_KEY: apiKey, CONTACT_EMAIL: recipient, TURNSTILE_SECRET_KEY: turnstileSecret } = env as CloudflareEnv & ContactEnv;
  if (!apiKey || !recipient || !turnstileSecret) return json({ error: "Email service unavailable" }, 503);
  ...
  if (!verificationResult.success) return json({ error: "Verification failed" }, 403);
